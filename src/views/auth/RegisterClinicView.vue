    ...
    <div class="flex flex-col gap-8 w-full max-w-md mx-auto md:mx-0">
      ...
      <form v-if="step === 1" class="flex flex-col gap-4 animate-fade-in" @submit.prevent="nextStep">
        <div class="grid grid-cols-2 gap-4">
          <InputText v-model="form.lastname" :placeholder="$t('auth.register_common.lastname')" class="p-3 bg-zinc-100 dark:bg-zinc-800 border-none rounded" required />
          <InputText v-model="form.firstname" :placeholder="$t('auth.register_common.firstname')" class="p-3 bg-zinc-100 dark:bg-zinc-800 border-none rounded" required />
        </div>

        <InputText v-model="form.email" type="email" :placeholder="$t('auth.register_common.email')" class="p-3 bg-zinc-100 dark:bg-zinc-800 border-none rounded" required />
        ...
        <Button type="submit" :label="$t('auth.register_common.next')" class="!bg-[#ff3b4e] !border-none !text-white !font-black !uppercase !py-3 !mt-4 rounded-md shadow-lg" />
      </form>

      <form v-else class="flex flex-col gap-4 animate-fade-in" @submit.prevent="handleRegister">

        <InputText v-model="form.clinic_name" :placeholder="$t('auth.register_clinic.fields.clinic_name')" class="p-3 bg-zinc-100 dark:bg-zinc-800 border-none rounded" required />

        <InputText v-model="form.rpps" :placeholder="$t('auth.register_clinic.fields.rpps')" class="p-3 bg-zinc-100 dark:bg-zinc-800 border-none rounded" required />
